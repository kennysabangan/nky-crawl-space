...
function buildEmailHtml(data: {
  ...
  const pathMatch = pagePath.match(/\/crawl-space\/([^/]+)/);
  ...
  const sourceLabel = isHomePage ? 'Homepage' : `${cityDisplay} — Crawl Space Repair`;
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const html = buildEmailHtml({
    siteName: site_name || 'Crawl Space Repair',
    fullName,
    email,
    phone: phone || '',
    comment: comment || '',
  ...
  try {
    await resend.emails.send({
      from: 'NKY Crawl Space Alerts <alerts@mursenmaintenance.com>',
